...
st.write("Enter your office and home address to calculate the travel time to this Introduction to Drawing Course")
...
if st.button('Calculate Travel Time'):
    if office_address:
        with st.spinner('Calculating...'):
            travel_time = calculate_travel_time(office_address, "Art Play - 16 horner square, E16EW")
    ...
    if home_address:
        with st.spinner('Calculating...'):
            travel_time = calculate_travel_time(home_address, "Art Play - 16 horner square, E16EW")
            if travel_time is not None:
