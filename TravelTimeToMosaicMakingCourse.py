...
st.write("Enter your office and home address to calculate the travel time to this Mosaic Making Course")
...
if st.button('Calculate Travel Time'):
    if office_address:
        with st.spinner('Calculating...'):
            travel_time = calculate_travel_time(office_address, "31 Jeffreys Rd, London SW4 6QU")
    ...
    if home_address:
        with st.spinner('Calculating...'):
            travel_time = calculate_travel_time(home_address, "31 Jeffreys Rd, London SW4 6QU")
            if travel_time is not None:
