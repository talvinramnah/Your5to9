...
st.write("Enter your office and home address to calculate the travel time to this Acrylic Painting Course")
...
if st.button('Calculate Travel Time'):
    if office_address:
        with st.spinner('Calculating...'):
            travel_time = calculate_travel_time(office_address, "Southgate methodist church - N14 6RS")
    ...
    if home_address:
        with st.spinner('Calculating...'):
            travel_time = calculate_travel_time(home_address, "Southgate methodist church - N14 6RS")
            if travel_time is not None:
